...
export function PointsManagementPage() {
  ...
  async function fetchAll() {
    ...
    if (error) {
      console.warn('[points-mgmt] order by points failed:', error.message, '| code:', error.code);
    }
    if (error && /column.*point(s|_period|_log)?.*does not exist/i.test(error.message)) {
      console.log('[points-mgmt] points column missing, fallback to name order');

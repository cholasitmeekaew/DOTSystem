...
import {
  Officer, ServiceRecord, DutyLog, Department, DEPARTMENT_LABELS, RANK_LABELS,
} from '../../lib/types';
import {
...
export function DepartmentBalancePage() {
  ...
  const [sroList, setSroList] = useState<{ service_record_id: string; officer_id: string }[]>([]);
  ...
  async function fetchAll() {
    setLoading(true);
    try {
      const [{ data: o }, { data: r }, { data: dl }, { data: sro }, configs] = await Promise.all([
      ...
      setSroList((sro ?? []) as { service_record_id: string; officer_id: string }[]);
  ...
  }, [selectedMonth, officers, dutyLogs, paidRecords, sroList, staffSharePercent, customBonus]);
  ...
  return (
    <FadeIn>
      ...
      {loading ? (
          ...
          <div className="card overflow-hidden">
            ...
            <div className="overflow-x-auto scrollbar-thin">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {payrollList.map((p) => (
                    <tr key={p.officer_id} className="border-t border-blue-900/30 hover:bg-navy-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-mono font-bold text-sm text-amber-400">
                        {p.total_net_payout.toLocaleString('th-TH')} BC
                      </td>
                    </tr>
                  ))}
         ...
         * ========================================================================= */
        <ClaimApprovalTable approverId={currentOfficer.id} officers={officers} />
      ) : (
        /* =========================================================================

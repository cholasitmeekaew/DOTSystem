...
const navItems: NavItem[] = [
  ...
  { id: 'manual', label: 'คู่มือการใช้งาน', icon: <BookOpen size={18} />, commissionerOnly: false },
  { id: 'my-points', label: 'คะแนน PPS ของฉัน', icon: <Award size={18} />, commissionerOnly: false },
  { id: 'points-management', label: 'จัดการคะแนน', icon: <Award size={18} />, commissionerOnly: true },
  { id: 'department-balance', label: 'การเงิน & สัดส่วน', icon: <Wallet size={18} />, commissionerOnly: true },
];
  ...
  return (
    <div className="min-h-[100dvh] bg-navy-900 flex">
      ...
      >
        ...
        <nav className="flex-1 min-h-0 px-3 py-4 space-y-0.5 overflow-y-auto overscroll-contain scrollbar-thin">
        ...
        <footer className="border-t border-blue-900/40 py-5 px-4 lg:px-6 mt-auto bg-navy-900/80 backdrop-blur-sm">
          <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-400">
            <div className="flex items-center gap-2">
              <img src={LOGO_URL} alt="DOT" className="h-5 w-auto" />
              <span className="font-semibold text-gray-300">กรมการขนส่งทางบก (DOT) · Bit Cities</span>
            </div>
            <div className="flex items-center gap-1.5 text-amber-400 font-medium bg-amber-500/10 border border-amber-500/25 px-3 py-1.5 rounded-full shadow-sm">
              <Newspaper size={14} className="text-amber-400" />
              <span>พาร์ทเนอร์กับ สำนักข่าว Bit news</span>
            </div>
            <div className="text-gray-500 text-[11px]">
              ระบบบริหารจัดการภายในสำหรับเจ้าหน้าที่
            </div>
          </div>
        </footer>
      </div>

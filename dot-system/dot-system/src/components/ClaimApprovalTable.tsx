...
export function ClaimApprovalTable({ approverId, officers }: Props) {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const [p, a] = await Promise.all([fetchPendingClaims(), fetchApprovedClaims()]);
      setPending(p);
      setApproved(a);
    } catch (e) {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <div className="card p-3 border-amber-500/30 bg-amber-500/5">
          <div className="flex items-center gap-2 text-xs text-amber-300">
            <HandCoins size={14} /> รออนุมัติ
          </div>
          <div className="text-xl font-black text-amber-300 font-mono mt-1">
            {pending.length} <span className="text-xs font-normal text-gray-400">รายการ</span>
          </div>
        ...
        <div className="card p-3 border-emerald-500/30 bg-emerald-500/5">
          <div className="flex items-center gap-2 text-xs text-emerald-300">
            <CheckCircle2 size={14} /> อนุมัติแล้ว · รอจ่าย
          </div>
          <div className="text-xl font-black text-emerald-300 font-mono mt-1">
            {approved.length} <span className="text-xs font-normal text-gray-400">รายการ</span>
          </div>

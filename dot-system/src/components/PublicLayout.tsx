...
import { Truck, Users, LogIn, Tag, MessageSquare, Siren, Menu, X, IdCard, BarChart3, ChevronRight, Trophy, ScrollText, Newspaper } from 'lucide-react';
      ...
      >
        ...
        <nav className="flex-1 min-h-0 px-3 py-4 space-y-0.5 overflow-y-auto overscroll-contain scrollbar-thin">
          {navLinks.map((link) => (
            ...
            >
              <span className={currentPage === link.id ? 'text-amber-400' : 'text-gray-500 group-hover:text-gray-300'}>
                {link.icon}
              </span>
              <span className="flex-1 text-left">{link.label}</span>
              {currentPage === link.id && <ChevronRight size={14} className="text-amber-400" />}
            </button>
          ))}
        </nav>
        ...
        <footer className="border-t border-blue-900/40 py-5 px-4 lg:px-6 mt-auto bg-navy-900/80 backdrop-blur-sm">
          <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-400">
            ...
            <div className="text-gray-500 text-[11px]">
              © {new Date().getFullYear()} Bit Cities DOT System. All rights reserved.
            </div>
          </div>
        </footer>
      </div>
